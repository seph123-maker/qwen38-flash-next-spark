...
| Context limit says 262,144 instead of the configured 500,000 | A long-context override was omitted or the runtime interprets it differently | Compare the full environment and RoPE arguments in `production.json`. The published launcher includes both. |
| Long prompt reports preemption | Not enough cache blocks were available at an allocation point | Record counts and request sizes. One occurred at 483k and three at 678k; it is an unresolved performance limitation. |
